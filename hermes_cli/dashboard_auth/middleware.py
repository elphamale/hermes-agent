...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    if provider.supports_password:
        ...
        # redirect flow to auto-initiate -- start_login() isn't implemented
        # for them and raises NotImplementedError, which /auth/login has no
        # reason to expect and previously surfaced as a raw 500 on every
        # unauthenticated document load. There's nothing to bounce to here;
        # fall through to the ordinary /login page, which renders this
        # provider's password form directly and POSTs to
        # /auth/password-login.
        return None
